...
(assert (= (listDeleteMinimum nil) Nothing))
(assert
...
(assert (= (maybeToList Nothing2) Nothing))
(assert
  (forall ((y Heap)) (= (maybeToList (Just2 y)) (Just (toList2 y)))))
(assert
...
(assert
  (forall ((l Heap) (y Nat) (r Heap))
    (= (deleteMinimum (Node l y r)) (Just2 (merge l r)))))
(assert (= (deleteMinimum Nil) Nothing2))
(assert-not
  (forall ((h Heap))
    (=> (heap h)
