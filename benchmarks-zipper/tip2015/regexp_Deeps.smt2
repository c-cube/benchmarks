...
(assert (= (deeps Nil) Nil))
(assert (= (deeps Eps) Nil))
(assert (forall ((a A)) (= (deeps (Atom a)) (Atom a))))
(assert
  (forall ((p R) (q R))
    (= (deeps (Plus p q)) (Plus (deeps p) (deeps q)))))
(assert
  (forall ((p2 R) (q2 R))
    (=> (= (and (eps p2) (eps q2)) true)
      (= (deeps (Seq p2 q2)) (Plus (deeps p2) (deeps q2))))))
(assert
  (forall ((p2 R) (q2 R))
    (=> (= (and (eps p2) (eps q2)) false)
      (= (deeps (Seq p2 q2)) (Seq p2 q2)))))
(assert (forall ((p3 R)) (= (deeps (Star p3)) (deeps p3))))
(assert-not
  (forall ((p R) (s list))
